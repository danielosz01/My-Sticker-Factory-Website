  ...
  var toolbar = function () {
    ...
    try {
      this.containerEl.append(
        `<div class="toolbar" id="toolbar"><div class="main-buttons"></div><div class="am-custom-shapes">
        <a href="/editor?shape=square">Square</a>
        <a href="/editor?shape=rectangle">rectangle</a>
        <a href="/editor?shape=circle">Circle</a>
        <a href="/editor?shape=oval">Oval</a>
        </div><div class="extended-buttons"></div></div>`
      );
      ...
      (() => {
        buttons.forEach((item) => {
          $(`${this.containerSelector} #toolbar .main-buttons`).append(
            `<button id="${item.name}" class="${
              item.name === "shapes" ? "active" : ""
            }">${item.icon}</button>`
          );
        });
          ...
          function () {
            ...
            if (id === "save") {
              ...
              <div class="custom-modal-content">
                ...
                <div class="button-download" id="pdf">Download as PDF</div>
              </div>
            </div>`);
              ...
              $(".custom-modal-container .button-download").click(function (e) {
                ...
                if (type === "pdf") {
                  ...
                  // var imgData = _self.canvas.toDataURL("image/jpeg", 1.0);
                  // var pdf = new jsPDF();
                  // pdf.addImage(imgData, "JPEG", 0, 0);
                  // pdf.save("download.pdf");

                  let div = $("#canvas-holder>.content");
                  html2canvas(div).then(function (canvas) {
                    console.log(canvas);
                    document.getElementById("outputtt").appendChild(canvas);
                    $("#outputtt canvas").attr("id", "weather");
                    ...
                    if (location.href.indexOf("rectangle") != -1) {
                      pdf.addImage(imgData, "JPEG", 35, 75);
                      pdf.save("download.pdf");
                    } else if (location.href.indexOf("oval") != -1) {
                      pdf.addImage(imgData, "JPEG", 47, 75);
                      pdf.save("download.pdf");
                    } else {
                      pdf.addImage(imgData, "JPEG", 65, 75);
                      pdf.save("download.pdf");
                    }
                    $("#outputtt canvas").remove("");
                  });
                }
              });
